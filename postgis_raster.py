...
class PostGISRasterHandler:
    ...
    def create_pixel_cache_table(self) -> bool:
        ...
        try:
            ...
            # PostgreSQL might have created it as either a constraint or an index
            drop_commands = [
            ...
            for cmd in drop_commands:
                try:
                    ...
                    # Ignore errors - constraint/index might not exist
                    pass
            
            # Create new unique index on (year, bounds_data) to allow multiple areas per year
            # This allows multiple polygons/rectangles per year, but prevents duplicates
            try:
                # Try using md5 hash for bounds_data (more efficient for large JSON)
                self.cursor.execute("""
                    DROP INDEX IF EXISTS pixel_cache_year_bounds_unique CASCADE;
                    CREATE UNIQUE INDEX pixel_cache_year_bounds_unique 
                    ON pixel_cache(year, md5(COALESCE(bounds_data, '')));
                """)
                self.connection.commit()
            except Exception as e:
                # If md5 doesn't work or bounds_data is NULL, try direct comparison
                try:
                    self.cursor.execute("""
                        DROP INDEX IF EXISTS pixel_cache_year_bounds_unique CASCADE;
                        CREATE UNIQUE INDEX pixel_cache_year_bounds_unique 
                    ...
                    # Continue anyway - the check in cache_pixel_data will handle duplicates
            
            # Create index on year for faster lookups
            self.cursor.execute("CREATE INDEX IF NOT EXISTS pixel_cache_year_idx ON pixel_cache(year);")
            self.cursor.execute("CREATE INDEX IF NOT EXISTS pixel_cache_bounds_type_idx ON pixel_cache(bounds_type);")
            
            self.connection.commit()
            print(f"✅ Created/updated pixel cache table")
            return True
    ...
    def cache_pixel_data(self, year: int, bounds: Dict[str, Any]) -> bool:
        ...
        try:
            # Check if data is already cached with matching bounds
            bounds_type = bounds.get('type', 'rectangle')
            bounds_json = json.dumps(bounds, sort_keys=True)
            
            check_sql = """
            SELECT id FROM pixel_cache 
            WHERE year = %s AND bounds_type = %s AND bounds_data::text = %s
            """
            self.cursor.execute(check_sql, (year, bounds_type, bounds_json))
            
            if self.cursor.fetchone():
                print(f"✅ Pixel data for year {year} with matching bounds already cached")
                return True
            ...
            # Use ON CONFLICT with the unique index on (year, md5(bounds_data))
            # PostgreSQL doesn't support ON CONFLICT with expressions directly,
            # so we'll use a different approach - check first, then insert/update
            bounds_json = json.dumps(bounds, sort_keys=True)
            
            # Check if entry exists with same year and bounds
            check_sql = """
            SELECT id FROM pixel_cache 
            WHERE year = %s AND bounds_data::text = %s
            """
            self.cursor.execute(check_sql, (year, bounds_json))
            existing = self.cursor.fetchone()
            
            if existing:
                # Update existing entry
                update_sql = """
                UPDATE pixel_cache SET
                    pixel_data = %s,
                    data_shape = %s,
                    bounds_type = %s,
                    total_pixels = %s,
                ...
                # Insert new entry - handle potential race condition
                insert_sql = """
                ...
                try:
                    ...
                    if 'unique' in str(insert_error).lower() or 'duplicate' in str(insert_error).lower():
                        print(f"⚠️ Insert failed (duplicate), updating instead...")
                        update_sql = """
                        UPDATE pixel_cache SET
                            pixel_data = %s,
                            data_shape = %s,
    ...
    def get_cached_pixel_data(self, year: int, bounds: Optional[Dict[str, Any]] = None, bounds_type: str = None) -> Optional[Tuple[np.ndarray, Dict[str, Any]]]:
        """Retrieve cached pixel data from database
        
        Args:
            year: Year of the data
            bounds: Optional bounds dict to match against cached bounds
            bounds_type: Optional bounds_type (will be inferred from bounds if not provided)
        """
        try:
            # Determine bounds_type if not provided
            if bounds_type is None:
                if bounds:
                    bounds_type = bounds.get('type', 'rectangle')
                else:
                    bounds_type = 'polygon'  # Default
            
            # Build query - if bounds provided, compare actual bounds_data
            if bounds:
                # Compare bounds by JSON matching
                bounds_json = json.dumps(bounds, sort_keys=True)
                select_sql = """
                ...
                WHERE year = %s AND bounds_type = %s AND bounds_data::text = %s
                """
                self.cursor.execute(select_sql, (year, bounds_type, bounds_json))
            else:
                # No bounds provided - get any cached data for this type (backward compatibility)
                select_sql = """
                SELECT pixel_data, data_shape, bounds_data, total_pixels, tree_pixels, coverage_percent,
                       visualization_image, geo_bounds
                FROM pixel_cache
            ...
            # Verify bounds match if bounds were provided
            if bounds:
                cached_bounds_json = result['bounds_data']
                try:
                    cached_bounds = json.loads(cached_bounds_json) if isinstance(cached_bounds_json, str) else cached_bounds_json
                    # Compare bounds (normalize for comparison)
                    if not self._bounds_match(bounds, cached_bounds):
                        print(f"⚠️ Cached bounds don't match requested bounds, skipping cache")
                        return None
                except:
                    pass
    ...
    def _bounds_match(self, bounds1: Dict[str, Any], bounds2: Dict[str, Any]) -> bool:
        """Check if two bounds dictionaries match (within tolerance for floating point)"""
        try:
            # Compare types
            if bounds1.get('type') != bounds2.get('type'):
                return False
            
            bounds_type = bounds1.get('type', 'rectangle')
            
            if bounds_type == 'polygon':
                # Compare polygon coordinates (normalize order)
                coords1 = bounds1.get('coordinates', [])
                coords2 = bounds2.get('coordinates', [])
                
                if len(coords1) != len(coords2):
                    return False
                
                # Compare coordinates with tolerance
                tolerance = 1e-6  # ~0.1 meter
                for c1, c2 in zip(coords1, coords2):
                    if abs(c1[0] - c2[0]) > tolerance or abs(c1[1] - c2[1]) > tolerance:
                        return False
                ...
                for key in ['west', 'east', 'north', 'south']:
                    if key in bounds1 and key in bounds2:
                        if abs(bounds1[key] - bounds2[key]) > 1e-6:
                            return False
                    elif (key in bounds1) != (key in bounds2):
                        return False
...
def get_tree_coverage_postgis(year: int, bounds: Optional[Dict[str, Any]] = None) -> Tuple[float, str]:
    """Get tree coverage using cached pixel data (fast) or COG file (slow fallback)
    
    Args:
        year: Year of the data
        bounds: Optional bounds dict (polygon or rectangle). If None, uses HUDSON_SQUARE_BOUNDS
    """
    try:
        from config import LIDAR_DATASETS, HUDSON_SQUARE_BOUNDS
        import numpy as np
        
        # Use provided bounds or fall back to default
        active_bounds = bounds if bounds is not None else HUDSON_SQUARE_BOUNDS
        
        # Try to get cached data first (FAST)
        # Now compares actual bounds, not just bounds_type
        handler = PostGISRasterHandler()
        if handler.connect():
            try:
                cached_result = handler.get_cached_pixel_data(year, active_bounds)
                ...
                if cached_result:
                    ...
                    if bounds:
                        print(f"⚠️ No cache found for drawn area, calculating from COG...")
                    else:
                        print(f"⚠️ No cache found, calculating from COG...")
        ...
        print(f"🔍 Reading from COG file for drawn area: {cog_url}")
            ...
            with rasterio.open(cog_url) as src:
                ...
                if active_bounds.get('type') == 'polygon':
                    ...
                    if 'west' in active_bounds:
                        raster_bounds = transform_bounds('EPSG:4326', src.crs, 
                                                       active_bounds['west'], active_bounds['south'],
                                                       active_bounds['east'], active_bounds['north'])
                    else:
                        bounds_rect = get_study_area_bounds()
                        raster_bounds = transform_bounds('EPSG:4326', src.crs, 
                                                       bounds_rect['west'], bounds_rect['south'],
                                                       bounds_rect['east'], bounds_rect['north'])
                ...
                if total_pixels > 0:
                    ...
                    # Cache this data for next time (with current bounds)
                    if handler.connect():
                        try:
                            handler.cache_pixel_data(year, active_bounds)
